...
ggplot() +
  ...
  theme(
    plot.title = element_text(size = 28, face = "bold", hjust = 0.5),  # Larger title
...
ggplot() +
  geom_sf(data = us_states, fill = "#4B6D34", color = "white") +
   geom_point(data = GT_stops, 
             aes(x = Longitude, y = Latitude),
             shape = 23, fill = "#C3A571", size = 3, color = "white") +
  geom_sf(data = PA, fill = "#C3A571", color = "white") +
  labs(
    title = "States Chairman Thompson Visited",
    subtitle = "Since Becoming Republican Leader of the Committee",
    x = NULL,
   ...
   ggrepel::geom_label_repel(
    data = GT_labels |>
      filter(State %in% c("Iowa", "Louisiana", "Minnesota", "Indiana", "Illinois", "Ohio")),
    aes(label = Purpose, x = Longitude, y = Latitude),
     min.segment.length = 0, 
    seed = 1234,
    force = 20,
     max.overlaps = Inf,
...
GT_stops <- read_excel("GT_stops.xlsx")
...
colnames(GT_stops)[2] <- "State"

GT_stops$State[is.na(GT_stops$State)] <- "State"
...
GT_labels$Purpose <- stringr::str_wrap(GT_labels$Purpose, 25)
```
...
ggplot() +
  geom_sf(data = us_states, fill = "#4B6D34", color = "white") +
   geom_point(data = GT_stops, 
             aes(x = Longitude, y = Latitude),
             shape = 23, fill = "#C3A571", size = 3, color = "black") +
  geom_sf(data = PA, fill = "#C3A571", color = "black") +
  labs(
    title = "States Chairman Thompson Visited",
    subtitle = "Since Becoming Republican Leader of the Committee",
    x = NULL,
